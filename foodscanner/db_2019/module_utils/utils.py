...
# ----------------------------------------------------------------------
# --------------- HIGHLIT MIN AND MAX VALUES PER INDEX -----------------
# ----------------------------------------------------------------------


def highlight_max(s):
    '''
    highlight the maximum in a Series darkorange.
    '''
    is_max = s == s.max()
...
def highlight_min(s):
    '''
    highlight the minimum in a Series darkorange.
    '''
    is_max = s == s.min()
    return ['background-color: darkorange' if v else '' for v in is_max]


def highlight_bests_metrics(df):
    """
    Highlight the highest values for index 2 and 4 and highlight the lowest values for index 0, 1, 4
    ...
    Return
    ------
    Return pandas.formats.style.Styler
    """
    return df.style.apply(highlight_max, subset=(df.index[[2, 4]], df.select_dtypes(float).columns), axis=1)\
        .apply(highlight_min, subset=(df.index[[0, 1, 3]], df.select_dtypes(float).columns), axis=1)


def train_test_df(df_1, df_2, column):
    df = pd.concat([df_1[[column]], df_2[[column]]], axis=1)
    df.index.name = column
    df.columns = ['Train', 'Test']
    return highlight_bests_metrics(df)
