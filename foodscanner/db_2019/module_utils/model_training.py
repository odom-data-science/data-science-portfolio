...
def training_models(x, y, models, col_name):
    """
    Takes x and y as parameters to train models
    x: np.array
        input variables
    y: np.array
        target variable(s)
    models: list of objects models
    """
    # create empty dataframe which groups together the metrics of all the prediction models
    final_df = pd.DataFrame()
    for model in models:
        # enter train and test set in each of the models
        train_model(model, x, y)
        dict_losses = compute_losses_reg(model, x, y)
